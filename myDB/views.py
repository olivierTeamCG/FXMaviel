...
from .forms import ContactFX
from .forms import ContactFrancois
import logging
...
def contactfx(request):
    ...
    if request.method == "POST":
        ...
        form = ContactFXForm(request.POST)
...
def detail(request, cid):
    contact = ContactFX.objects.get(pk=cid)
...
class ContactFrancoisForm(ModelForm):
    def __init__(self, *args, **kwargs):
        """
        Surcharge de l'initialisation du formulaire
        """
        ...
        self.helper.add_input(Submit('submit', 'Submit')) 
    class Meta:
        model = ContactFrancois
        #fields = ('nom', 'prenom', 'date_naissance','lieu_naissance','situation_familiale','nb_enfants','portable','email','adresse','cp','ville','recommande_par','vous_ne_transpirez_jamais')
        
        fields = '__all__'
        widgets = {'message': Textarea(attrs={'cols': 60, 'rows': 10}),}

def contactfrancois(request):
    # on instancie un formulaire
    form = ContactFrancoisForm()
    # on teste si on est bien en validation de formulaire (POST)
    if request.method == "POST":
        # Si oui on récupère les données postées
        form = ContactFrancoisForm(request.POST)
        # on vérifie la validité du formulaire
        logging.debug("form.is_valid()")
        logging.debug(form.is_valid())
        if form.is_valid():
...
def detail(request, cid):
    contact = ContactFrancois.objects.get(pk=cid)
    context = {'pers': contact}
    return render(request,'detail.html', context)
